...
#pragma mark DateFormat for Export

+(NSString*)getExportDateFormat{
	NSUserDefaults* prefs = [NSUserDefaults standardUserDefaults];
	NSString* dateFormat = [prefs stringForKey:@"exportdateformat"];
	
	if(dateFormat == nil || [dateFormat isEqualToString:@""]){
		dateFormat = NSLocalizedString(@"MM/dd/yyyy",@"DateFormatter");
        [CacheDataManager setExportDateFormat:dateFormat];
	}
...
+(void)setExportDateFormat:(NSString*)dateFormat{
	NSUserDefaults* prefs = [NSUserDefaults standardUserDefaults];
	[prefs setObject:dateFormat forKey:@"exportdateformat"];
	[prefs synchronize];
}
